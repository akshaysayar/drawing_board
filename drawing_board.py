    ...
    while True:
        ...
        if results.multi_hand_landmarks:
            ...
            for handLms in results.multi_hand_landmarks:
                ...
                dst = math.sqrt(((thumb_x-index_x)*(thumb_x-index_x))+((thumb_y-index_y)*(thumb_y-index_y)))
                mid_x= int((thumb_x+index_x)/2)
                mid_y = int((thumb_y+index_y)/2)
                z = (thumb_z + index_z)*-500
...
def run_alone(LReg):
    ...
    while True:
        ...
        try:
            for pt in pts:    
                for i in range(len(pt)-1):
                    cv2.line(img, (pt[i][0]   ,  pt[i][1]), (pt[i+1][0],pt[i+1][1]) , color_in_use, thickness=2)
        except:
            print("not working")
        cv2.imshow("Image", img)
        cv2.waitKey(1)
